...
function RiskSegments({ segments }: { segments: Extract<ApprovalCardProps['reason'], { kind: 'risk' }>['segments'] }): React.ReactElement {
  return (
    <ul style={{ listStyle: 'none', marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
      {segments.map((seg, i) => {
        const cls = seg.color === 'red' ? 'risk-red' : seg.color === 'yellow' ? 'risk-yellow' : 'risk-green';
        const mark = seg.color === 'red' ? '✗' : seg.color === 'yellow' ? '⚠' : '·';
        return (
          <li key={i} className={`risk-segment ${cls}`}>
            {mark} [{seg.risk}] {seg.text}
            {seg.matchedRule ? <span style={{ marginLeft: 4, opacity: 0.6 }}>({seg.matchedRule})</span> : null}
          </li>
        );
      })}
    </ul>
  ...
  return (
    <div className="approval-card">
      ...
      {props.reason.kind === 'policy' ? (
        <div className="approval-body">{props.reason.detail} <span style={{ opacity: 0.6 }}>[{props.reason.ruleId}]</span></div>
      ) : null}
      {props.reason.kind === 'manual' ? <div className="approval-body">{props.reason.note}</div> : null}

      {hunks.length > 0 ? (
        <div style={{ marginTop: 12, border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border-light)', padding: '6px 12px', fontSize: 12, color: 'var(--text-2)' }}>
            <span>Diff 审阅：{props.diff?.file}</span>
            <button style={{ background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: 12, textDecoration: 'underline' }}
              onClick={() => setSelectedHunks(selectedHunks.size === hunks.length ? new Set() : new Set(hunks.map((h) => h.id)))}>
              {selectedHunks.size === hunks.length ? '全不选' : '全选'}
            </button>
          </div>
          <ul style={{ maxHeight: 256, overflowY: 'auto', listStyle: 'none' }}>
            {hunks.map((h) => (
              <li key={h.id} style={{ cursor: 'pointer', borderBottom: '1px solid var(--border-light)', padding: '6px 12px', fontSize: 12 }}
                onClick={() => toggle(h.id)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input type="checkbox" readOnly checked={selectedHunks.has(h.id)} />
                  <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-2)' }}>
                    {h.id} · {h.kind} · L{h.oldRange.startLine}-{h.oldRange.endLine} → L{h.newRange.startLine}-{h.newRange.endLine}
                  </span>
                </div>
                <pre style={{ marginTop: 4, overflowX: 'auto', fontSize: 11, lineHeight: 1.4, fontFamily: 'var(--font-mono)' }}>
                  {h.oldLines.map((l, i) => (
                    <div key={`o${i}`} style={{ color: 'var(--danger)', opacity: 0.8 }}>- {l}</div>
                  ))}
                  {h.newLines.map((l, i) => (
                    <div key={`n${i}`} style={{ color: 'var(--success)', opacity: 0.8 }}>+ {l}</div>
                  ))}
                </pre>
              </li>
            ))}
      ...
      <div className="approval-actions" style={{ marginTop: 12 }}>
        <button className="btn-approve" onClick={() =>
          props.onResolve('allow', { ...(hunks.length ? { hunkIds: [...selectedHunks] } : {}), onceForSession })
        }>
          批准{hunks.length ? `（${selectedHunks.size} 个 hunk）` : ''}
        </button>
        <button className="btn-deny" onClick={() => props.onResolve('deny')}>拒绝</button>
