...
export function AnnotationOverlay(props: AnnotationOverlayProps): React.ReactElement {
  ...
  const onMouseDown = (e: React.MouseEvent<HTMLCanvasElement>): void => {
    const pos = getPos(e);
    if (tool === 'text') { setTextInput({ x: pos.x, y: pos.y, value: '' }); return; }
    setDrawing(true);
    setStart(pos);
  };
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLCanvasElement>): void => {
    ...
    const img = bgRef.current;
    if (img && img.complete) ctx.drawImage(img, 0, 0, canvasRef.current!.width, canvasRef.current!.height);
    for (const a of annotations) drawAnnotation(ctx, a);
    drawAnnotation(ctx, { tool, x: start.x, y: start.y, width: pos.x - start.x, height: pos.y - start.y, color });
  };
  ...
  const onMouseUp = (e: React.MouseEvent<HTMLCanvasElement>): void => {
    ...
    setDrawing(false);
    const pos = getPos(e);
    const annotation: Annotation = { tool, x: start.x, y: start.y, width: pos.x - start.x, height: pos.y - start.y, color };
  ...
  const confirmText = (): void => {
    ...
    setAnnotations((prev) => [...prev, { tool: 'text', x: textInput.x, y: textInput.y, width: 0, height: 0, color, text: textInput.value }]);
    setTextInput(null);
  };
    ...
    <div className="annotation-overlay">
      <div className="annotation-toolbar">
        ...
        {(['rect', 'arrow', 'text', 'highlight'] as AnnotationTool[]).map((t) => (
          <button key={t} className={`btn-sm ${tool === t ? 'primary' : ''}`} onClick={() => setTool(t)}>
        ...
        {COLORS.map((c) => (
          <button key={c} style={{ height: 24, width: 24, borderRadius: 'var(--radius-xs)', border: `2px solid ${color === c ? 'white' : 'transparent'}`, background: c, cursor: 'pointer' }}
            onClick={() => setColor(c)} />
        ))}
        <span style={{ margin: '0 8px', color: 'var(--text-3)' }}>|</span>
        <button className="btn-sm" onClick={() => setAnnotations((prev) => prev.slice(0, -1))}>撤销</button>
        <button className="btn-sm" onClick={() => setAnnotations([])}>清空</button>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button className="btn-sm" onClick={props.onCancel}>取消</button>
          <button className="btn-sm primary" onClick={handleConfirm}>确认并保存到输入框</button>
        </div>
      </div>
      <div className="annotation-canvas-area">
        <canvas
          ...
          style={{ maxHeight: '100%', maxWidth: '100%', cursor: 'crosshair', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-lg)' }}
          onMouseDown={onMouseDown}
          onMouseMove={onMouseMove}
          onMouseUp={onMouseUp}
        />
        {textInput ? (
          <div style={{ position: 'absolute', left: `${(textInput.x / (props.width || 1200)) * 100}%`, top: `${(textInput.y / (props.height || 800)) * 100}%` }}>
            <input autoFocus className="input-field" placeholder="输入标注文字…"
...
function drawAnnotation(ctx: CanvasRenderingContext2D, a: Annotation): void {
  ...
  switch (a.tool) {
    case 'rect': ctx.strokeRect(a.x, a.y, a.width, a.height); break;
    case 'arrow': drawArrow(ctx, a.x, a.y, a.x + a.width, a.y + a.height); break;
    case 'highlight': ctx.globalAlpha = 0.3; ctx.fillRect(a.x, a.y, a.width, a.height); ctx.globalAlpha = 1; break;
    case 'text': ctx.fillText(a.text ?? '', a.x, a.y); break;
  }
}
