...
export function TokenCounter(props: TokenCounterProps): React.ReactElement | null {
  ...
  return (
    <span className="token-display">
      {running ? <span className="status-dot running" /> : null}
      <span className="arrow-up" title={`input tokens: ${usage.inputTokens}`}>
        ↑{fmt(usage.inputTokens)}
      </span>
      <span className="arrow-down" title={`output tokens: ${usage.outputTokens}`}>
      ...
      {showCostBar ? (
        <span className="cost-bar" title={`成本 ${costPercent.toFixed(0)}%`}>
          <span
            ...
            style={{ width: `${Math.max(2, costPercent)}%`, background: barColor }}
          />
        </span>
      ) : null}
    </span>
